...
def subs_asc(L, b):
    ...
    assert np.allclose(L, np.tril(L), atol=1e-7), "L nu este superior triunghiulara!"
...
def fact_lu(A):
    ...
    for i in range(n-1):
        ...
        if not np.any(A[i:, i]):  # Verifica daca sunt zerouri in vectorul de sub pivot (inclusiv)
            ...
            p = np.argmax(np.abs(A[i:, i]))  # Pozitia primului element nenul din slice-ul coloanei i
...
def fact_qr(A):
    ...
    assert np.abs(np.linalg.det(A)) > 0, "Matricea nu este inversabila"
