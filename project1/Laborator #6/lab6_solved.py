...
def ex3():
    ...
    for i in range(len(inter_lagr_direct)):
        inter_lagr_direct[i] = interp_directa(x_clicks, y_clicks, domain[i])

    inter_spline_liniara = np.zeros_like(domain)  # TODO: To be modified    - Am modificat
    for i in range(len(inter_spline_liniara)):
        inter_spline_liniara[i] = spline_liniara(x_clicks, y_clicks, domain[i])
